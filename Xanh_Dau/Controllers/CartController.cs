...
public class CartController : Controller
{
    ...
    public async Task<IActionResult> AddCustomProductToCart(int customProductId)
    {
        try
        {
            var customerId = getUserId();
            ...
            await _cartRepository.AddCustomProductToCartAsync(cart.CartId, customProductId);
        
            TempData["Success"] = "Đã thêm sản phẩm tùy chỉnh vào giỏ hàng!";
            return RedirectToAction("CartDetail", "Home");
        }
        catch (Exception ex)
        {
            TempData["Error"] = "Có lỗi xảy ra khi thêm vào giỏ hàng.";
            return RedirectToAction("Customize", "CustomProduct");
        }
    }
